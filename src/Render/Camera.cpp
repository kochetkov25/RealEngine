...
void Camera::updateProjMat() {
  ...
  switch (_projectionMode) {
    ...
    case Render::Camera::ProjectionMode::ORTHOGRAPHIC_CENTER:
      _projMat = glm::ortho(-_windowWidth / 2.f, _windowWidth / 2.f, -_windowHeight / 2.f, _windowHeight / 2.f,
                            _nearPlane, _farPlane);
      break;
    case Render::Camera::ProjectionMode::ORTHOGRAPHIC_LEFT_BOT:
      _projMat = glm::ortho(0.f, _windowWidth, 0.f, _windowHeight, _nearPlane, _farPlane);
      break;
    default:
      _projMat = glm::perspective(glm::radians(45.f), aspect, _nearPlane, _farPlane);
      break;
...
void Camera::setSensitivity(const float sensitivity) { _sensitivity = (sensitivity >= 1) ? 1 : sensitivity; }
}  // namespace Render
