...
void Render::VertexArray::addBuffer(VertexBuffer& vertexBuffer) {
  ...
  for (const VertexBuffer::BufferElement& element : vertexBuffer._elements) {
    ...
    if (componentType == GL_UNSIGNED_INT) {
      glVertexAttribIPointer(_layoutLocation, element.size, componentType, vertexBuffer.getStride(),
                             reinterpret_cast<const void*>(element.offset));
    } else {
      glVertexAttribPointer(_layoutLocation, element.size, componentType, GL_FALSE, vertexBuffer.getStride(),
                            reinterpret_cast<const void*>(element.offset));
    }
