 ...
 public:
  struct subTexture2D {
    ...
    subTexture2D(const glm::vec2 &leftbottomUV, const glm::vec2 &rightTopUV)
        : _leftBottomUV(leftbottomUV), _rightTopUV(rightTopUV) {}
    subTexture2D() : _leftBottomUV(0.f), _rightTopUV(1.f) {}
  };

  Texture2D(const GLuint width, const GLuint height,
            const unsigned char *textureData, const unsigned int channels = 4,
            const GLenum filter = GL_LINEAR,
            const GLenum wrapMode = GL_CLAMP_TO_EDGE);

  Texture2D(const GLuint width, const GLuint height, const float *textureData,
            const unsigned int channels = 4, const GLenum filter = GL_LINEAR,
            const GLenum wrapMode = GL_CLAMP_TO_EDGE);
  void bindTexture2D(const unsigned short texBlock = 0);

  void addSubTexture2D(std::string textureName, const glm::vec2 &leftBottomUV,
