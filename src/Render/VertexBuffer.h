...
public:
  ...
  template <typename _T> void updateData(const std::vector<_T> &data) {
  ...
  void updateSubData(const _T *data, size_t count, size_t offset = 0) {
    bind();
    glBufferSubData(GL_ARRAY_BUFFER, offset * sizeof(_T), count * sizeof(_T),
                    data);
  }

  // Update buffer sub-data with raw pointer and byte size
  void updateSubDataRaw(const void *data, size_t sizeBytes,
