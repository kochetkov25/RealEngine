...
class VertexBuffer {
 public:
  ...
  struct BufferElement {
    ...
    BufferElement(_e_DataType type) : dataType(type), componentType(GL_FLOAT), size(0), offset(0) {};
  };

  // Constructor and destructor (copies layout so caller can call upload() again)
  template <typename _T>
  VertexBuffer(const std::vector<_T> &data, const std::vector<BufferElement> &elements, _e_Usage usage = _e_Usage::Static)
      : _elements(elements), _usage(usage) {
    size_t offset = 0;
  ...
  // Update buffer data (recreates buffer)
  template <typename _T>
  ...
  // Update buffer sub-data (updates existing buffer without recreation)
  // More efficient for frequent updates of instance/transform data
  template <typename _T>
  void updateSubData(const _T *data, size_t count, size_t offset = 0) {
    bind();
    glBufferSubData(GL_ARRAY_BUFFER, offset * sizeof(_T), count * sizeof(_T), data);
  }

  // Update buffer sub-data with raw pointer and byte size
  void updateSubDataRaw(const void *data, size_t sizeBytes, size_t offsetBytes = 0) {
    bind();
    glBufferSubData(GL_ARRAY_BUFFER, static_cast<GLintptr>(offsetBytes), static_cast<GLsizeiptr>(sizeBytes), data);
  }
